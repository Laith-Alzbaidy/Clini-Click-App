...
const Practitioner = () => {
  ...
  const handleConfirm = () => {
    ...
    const commonParams = `treatmentId=${subcategory}&practitionerId=${practitionerId}&date=${date}&timeSlotId=${selectedTime}`;
    ...
    if (
      ...
      router.push(`/login?${commonParams}`);
    } else {
      router.push(`/payment?${commonParams}`);
    }
  };
